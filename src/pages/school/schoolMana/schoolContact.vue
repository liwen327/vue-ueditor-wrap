  ...
  <div class="detail_wrap">
    <el-form :model="detailRuleForm"
      ...
      <div class="form-block">
        <h2 class="form-title">学校联系信息</h2>
      </div>

      <el-form-item label="内容"
                    prop="schoolContacts"
                    class="ueditor_wrap">
        <vue-ueditor-wrap v-model="detailRuleForm.schoolContacts"
                          :destroy="true"
                          ref="editor"
      ...
      optSchoolService.getEditSchoolList(parseInt(this.detailRuleForm.schoolId)).then((result) => {
        if (result.retCode === '00000') {
          ...
          if (result.vo.schoolCtd !== null) {
            if (result.vo.schoolCtd.schoolContacts) {
              this.detailRuleForm.schoolContacts = result.vo.schoolCtd.schoolContacts
        ...
        if (valid) {
          ...
          optSchoolService.editSchool(detailParams).then((result) => {
            if (result.retCode === '00000') {
              ...
              // this.$router.replace('/school/contact')
              location.reload()
