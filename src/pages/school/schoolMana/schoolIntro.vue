  ...
  <div class="detail_wrap">
    <el-form :model="detailRuleForm"
      ...
      <div class="form-block">
        <h2 class="form-title">学校简介</h2>
      </div>

      <el-form-item label="内容"
                    prop="schoolItdc"
                    class="ueditor_wrap">
        <vue-ueditor-wrap v-model="detailRuleForm.schoolItdc"
                          :destroy="false"
                          ref="editor"
                          mode="observer"
      ...
      optSchoolService.getEditSchoolList(parseInt(this.detailRuleForm.schoolId)).then((result) => {
        if (result.retCode === '00000') {
          ...
          if (result.vo.schoolCtd !== null) {
            if (result.vo.schoolCtd.schoolItdc) {
              this.detailRuleForm.schoolItdc = result.vo.schoolCtd.schoolItdc
            }
        ...
        if (valid) {
          ...
          optSchoolService.editSchool(detailParams).then((result) => {
            if (result.retCode === '00000') {
              ...
              // this.$router.replace('/school/introdution')
            } else {
              this.$message({
